...
class AdminShopController extends Controller
{
  ...
  public function getShopsWaitingApprove(Request $request)
  {
    // Lấy tham số start_date và end_date từ request, nếu không có thì sử dụng ngày hiện tại
    $startDate = $request->query('start_date', Carbon::today()->toDateString());
    $endDate = $request->query('end_date', Carbon::today()->toDateString());
    ...
    // Lấy danh sách các shop có tài khoản đang chờ approved
    $shopsQuery = Shop::whereHas('account', function ($query) use ($accountIds) {
      $query->whereIn('accounts.id', $accountIds);
    });
    ...
    // Trả về JSON response
    return response()->json([
      'message' => 'Shops waiting approve retrieved successfully!',
      'status' => 200,
      'page_number' => $pageNumber,
      'num_of_page' => $numOfPage,
      'total_pages' => $totalPages,
  ...
  // Phê duyệt shop
  public function approveShop($account_id)
  {
  ...
  // Restore shop
  public function restoreShop($account_id)
  {
    // Tìm tài khoản với account_id
    $account = Account::find($account_id);
